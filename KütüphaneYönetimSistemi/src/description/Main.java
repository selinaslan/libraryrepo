...
public class Main {
	...
	public static void yaz(long tc) {
		...
		String sparqlTxt = "PREFIX xsd:<http://www.w3.org/2001/XMLSchema#> "
				...
				+ "?s library:tc  ?tcc." + "?s foaf:family_name ?soyad ."
		...
		ResultSet ppersonResultSet = LibraryStore.getInstance()
		...
		while (ppersonResultSet.hasNext()) {
			...
			QuerySolution querySolution = (QuerySolution) ppersonResultSet
					.next();
			...
			// String friendName =
			// querySolution.getLiteral("friendName").getString();
			// System.out.println(name + "    " + soyad+"    " +" sifre:"+
			// password + " email  :  "+ email+" friend name: "+friendName) ;
			System.out.println("  !!   ad:" + name + " soyad:  " + soyad
					+ "  tcValue :  " + tcValue + " þifre   : " + password);
		}
	...
	public static void kitapYaz(int isbn) {
		...
		String sparqlTxt = "PREFIX xsd:<http://www.w3.org/2001/XMLSchema#> "
				...
				+ " ?s library:isbn ?b." + "?s library:author ?author ." + "}";

		ResultSet kitapResultSet = LibraryStore.getInstance()
				.queryModelAsSelect(sparqlTxt);

		System.out.println(sparqlTxt);
		System.out
				.println("!!!!!!!!!!!!kitap yazdýrýlýyor!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
	...
	public static void main(String[] args) {
		...
		String sparqlTxt = "PREFIX rdf:<" + RDF.getURI() + "> "
				...
				+ "SELECT * WHERE {" + "?s rdf:type foaf:Person. "
				+ "?s foaf:family_name ?soyad ."
				+ "?s library:password ?password ." + "?s foaf:name ?name."
				+ "?s library:email ?mail." + "?s foaf:knows ?friend. "
				+ "?friend foaf:name ?friendName ."
				+ "?friend library:email ?friendMail ."
				+ "?s library:read ?read ." + "?read library:title ?title ."
				+ "?s library:tc ?tc}";

		String txt2 = "PREFIX rdf:<" + RDF.getURI() + "> " + "PREFIX foaf:<"
				+ FOAF.getURI() + "> " + "PREFIX library:<"
				+ OntologyConstants.ONTOLOGY_BASE_URI + "> "
				+ "SELECT * WHERE {" + "?s library:read ?read ."
		...
		while (personResultSet.hasNext()) {
			QuerySolution querySolution = (QuerySolution) personResultSet
					.next();
			String name = querySolution.getLiteral("name").getString();
			...
			System.out.println("*******************************");
			System.out.println(name + "    " + soyad + "    " + tcValue
					+ " sifre:" + password + " email  :  " + email
					+ "   arkadaþý: " + friend + "    :" + friendm
					+ "  okuduðu kitap:  " + title);
		...
		ResultSet bookkResultSet = LibraryStore.getInstance()
				.queryModelAsSelect(sparqlKitap);
		...
		while (bookkResultSet.hasNext()) {
			...
			String publicationDate = querySolution
					.getLiteral("publicationDate").getString();
			int bookCount = querySolution.getLiteral("bookCount").getInt();
			System.out.println(querySolution.getLiteral("isbn"));
			System.out.println(title + "    " + isbn + "    " + " Date  :  "
					+ publicationDate + "   adet: " + bookCount + " author: "
					+ authorList);
		...
		// ilk admini ekleme
		/*
		 * String tccc="66666666666"; Model adminModel = IndividualCreator
		 * .createAdmin("Admin", "Admin",Long.parseLong(tccc),"12345",
		 * "admin@gmail.com", "admin");
		 * 
		 * LibraryStore.getInstance().addResourceModel(adminModel);
		 */
		String tcNo = "122";
		Resource user = LibraryStore.getInstance().findUser(tcNo);
		List<Suggestion> suggestions = LibraryStore.suggestBook(user);
